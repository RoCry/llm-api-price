...
- **SOTA rule** — a regex describing a frontier model family (from `js/config.js`); the latest priced model group matching the rule (by version comparison) is marked SOTA. Default view shows SOTA only. Curation criterion: per-vendor frontier families; external leaderboards are triggers for noticing missing families, not the source of truth.
- **Version order** — how version tails in normalized names are compared for SOTA selection: dotted tokens are decimal fractions (`4.20` = 4.2 < `4.5`); dash-separated tokens are integer sequences (`4-10` > `4-8`).
